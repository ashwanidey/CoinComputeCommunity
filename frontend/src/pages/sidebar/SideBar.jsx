...
const SideBar = (props) => {
  ...
  const {isLoggedIn,setIsLoggedIn,setUser,deleteUser} = useContext(UserContext)
  useEffect(() => {
   initFlowbite();
   }, []);
  return (
...
<aside id="cta-button-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full lg:translate-x-0" aria-label="Sidebar">
   ...
   <button type="button" data-drawer-hide="cta-button-sidebar" aria-controls="cta-button-sidebar" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-6 end-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white lg:hidden" >
      ...
      <ul class="space-y-2 font-medium pl-0">
         ...
         <li>
            <a href="#" class="text-decoration-none flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group"  onClick={() =>{setShowModal(true); setIsLogin(false)}}> 
               <svg class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
